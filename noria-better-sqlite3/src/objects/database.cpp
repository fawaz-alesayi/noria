...
Database::~Database() {
	...
	CloseHandles();
	// noria may have been deleted by CloseHandles(), guard against double-delete
	if (noria) {
		delete noria;
		noria = nullptr;
	}
...
void Database::CloseHandles() {
	if (open) {
		...
		stmts.clear();
		backups.clear();
		// Delete noria BEFORE closing SQLite handle, since Noria holds reference to db_handle
		if (noria) {
			delete noria;
			noria = nullptr;
		}
...
INIT(Database::Init) {
	...
	SetPrototypeMethod(isolate, data, t, "cacheStats", JS_cacheStats);
	SetPrototypeMethod(isolate, data, t, "flushCache", JS_flushCache);
...
NODE_METHOD(Database::JS_exec) {
	...
	// Notify Noria of changes for CDC propagation
	if (status == SQLITE_OK && db->noria) {
		db->noria->NotifyChange();
	}
...
NODE_METHOD(Database::JS_cacheStats) {
	Database* db = Unwrap<Database>(info.This());
	REQUIRE_DATABASE_OPEN(db);
	UseIsolate;
	UseContext;

	NoriaCacheStats stats = db->noria->GetStats();

	v8::Local<v8::Object> result = v8::Object::New(isolate);
	result->Set(ctx, InternalizedFromLatin1(isolate, "cacheHits"),
		v8::Number::New(isolate, static_cast<double>(stats.cache_hits))).FromJust();
	result->Set(ctx, InternalizedFromLatin1(isolate, "cacheMisses"),
		v8::Number::New(isolate, static_cast<double>(stats.cache_misses))).FromJust();
	result->Set(ctx, InternalizedFromLatin1(isolate, "totalRows"),
		v8::Number::New(isolate, static_cast<double>(stats.total_rows))).FromJust();
	result->Set(ctx, InternalizedFromLatin1(isolate, "viewCount"),
		v8::Int32::New(isolate, stats.view_count)).FromJust();
	result->Set(ctx, InternalizedFromLatin1(isolate, "nodeCount"),
		v8::Int32::New(isolate, stats.node_count)).FromJust();

	info.GetReturnValue().Set(result);
}

NODE_METHOD(Database::JS_flushCache) {
	Database* db = Unwrap<Database>(info.This());
	REQUIRE_DATABASE_OPEN(db);
	REQUIRE_DATABASE_NOT_BUSY(db);
